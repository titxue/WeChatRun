...
def send(user, passwd, step):
    ...
    finddate = re.compile('.*?date%22%3A%22(.*?)%22%2C%22data.*?')
    findstep = re.compile('.*?ttl%5C%22%3A(.*?)%2C%5C%22dis.*?')
    ...
    try:
        response = requests.post(url, data=data, headers=head).text
        print(response)
